...
def plot_result(*args):
    ...
    names_lst = [f'Image {number_image}' for number_image in range(1, number_images)]
...
def plot_histogram(image):
    fig, axis = plt.subplots(nrows=1, nclos=3, figsize=(12, 4), sharex=True, sharey=True)
    color_lst = ['red', 'green', 'blue']
    for index, (ax, color) in enumerate(zip(axis, color_lst)):
        ax.set_title(f'{color.title()} histogram.')
        ax.histogram(image[:, :, index].ravel(), bins=256, color=color, alpha=0.8)
    fig.tight_layout()
    plt.show()
